...
async function showCelebrationPopup() {
  ...
  try {
    ...
    const savedName = localStorage.getItem('codebloom-pet-name') || 'Name your pup:';
...
async function celebrate() {
  ...
  spawnClickableTreats(celebration.treats);
}
...
function spawnClickableTreats(treats) {
  ...
  treats.forEach((emoji, idx) => {
    if (idx >= positions.length) return; // Don't exceed available positions
    ...
    const x = dogCenterX + pos.x;
    const y = dogCenterY + pos.y;
